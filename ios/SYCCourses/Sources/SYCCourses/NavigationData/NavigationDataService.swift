...
final class NavigationDataService: ObservableObject {
    ...
    var hasNavigationInputOwners: Bool {
        !inputOwners.isEmpty
    }
    ...
    func startNavigationInput(for owner: InputOwner) {
        ...
        case .connected, .connecting, .receiving:
            return
        case .disconnected, .stale, .error, .invalidFix:
            Task { await actisenseProvider.connect() }
        }
    }
    ...
    func isNavigationInputActive(for owner: InputOwner) -> Bool {
        inputOwners.contains(owner)
    }
    ...
    func activeFix(iPhoneFix: NavigationFix?, now: Date = Date()) -> NavigationFix? {
        actisenseProvider.refreshFreshness(now: now)
